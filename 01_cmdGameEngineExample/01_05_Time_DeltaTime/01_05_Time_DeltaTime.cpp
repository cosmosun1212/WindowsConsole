...
int main()
{
    ...
    while (g_Input.keyState[VK::Esc] != KEY_DOWN)
    {
        ...
        if (g_Input.keyState[VK::Up]    == KEY_DOWN || g_Input.keyState[VK::Up]    == KEY_HOLD) playerY -= moveSpeed * ((btnFrameLimit)?g_Time.deltaTime:1.f);
        if (g_Input.keyState[VK::Down]  == KEY_DOWN || g_Input.keyState[VK::Down]  == KEY_HOLD) playerY += moveSpeed * ((btnFrameLimit)?g_Time.deltaTime:1.f);
        if (g_Input.keyState[VK::Left]  == KEY_DOWN || g_Input.keyState[VK::Left]  == KEY_HOLD) playerX -= moveSpeed * ((btnFrameLimit)?g_Time.deltaTime:1.f);
        if (g_Input.keyState[VK::Right] == KEY_DOWN || g_Input.keyState[VK::Right] == KEY_HOLD) playerX += moveSpeed * ((btnFrameLimit)?g_Time.deltaTime:1.f);
